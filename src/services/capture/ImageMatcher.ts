...
interface CardHash {
  id: string;
  hash: string;
  // optional metadata (present if generated by enhanced script)
  dbfId?: number;
  name?: string;
  mana?: number;
  rarity?: string;
...
export class ImageMatcher extends EventEmitter {
  ...
  private async loadCardHashes(): Promise<void> {
    try {
      // Support both legacy (card-hashes.json) and new (card_hashes.json) filenames
      const legacyPath = path.join(__dirname, '../../../data/hashes/card-hashes.json');
      const newPath    = path.join(__dirname, '../../../data/hashes/card_hashes.json');

      let hashesPath = '';
      if (fs.existsSync(newPath)) {
        hashesPath = newPath;
      } else if (fs.existsSync(legacyPath)) {
        hashesPath = legacyPath;
      }

      if (hashesPath) {
        this.cardHashes = JSON.parse(fs.readFileSync(hashesPath, 'utf8'));
        this.hashesLoaded = true;
        logger.info('Card hashes loaded successfully', { count: this.cardHashes.length, path: path.basename(hashesPath) });
      } else {
        logger.warn('Card hashes file not found in expected locations', { legacyPath, newPath });
        logger.info('Run "npm run generate-hashes" to create the database');
    ...
    try {
      ...
      if (bestMatch && bestSimilarity >= this.HASH_MATCH_THRESHOLD) {
        ...
        return {
          region: captureResult.region,
          cardId: bestMatch.id,
          dbfId: bestMatch.dbfId ?? null,
          name: bestMatch.name ?? null,
          confidence: bestSimilarity,
          mana: bestMatch.mana ?? null,
